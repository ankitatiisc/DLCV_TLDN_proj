...
def create_nerf(args, time_latent):
...
def train():
    ...
    #Call time latent here
    time_codes = make_time_latent()
    time_codes.to(device)

    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_nerf(args, time_codes)
    global_step = start
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_weights==0:
            ...
            save_dict = {
                ...
                'optimizer_code_state_dict': optimizer_latent_code.state_dict()
            }
            np.save('mean_latent.npy', mean_time_latent/wt_mean_time_latent)
            if render_kwargs_train['network_fine'] is not None:
                save_dict['network_fine_state_dict'] = render_kwargs_train['network_fine'].state_dict()
            save_dict['latent_code'] = time_codes.state_dict()
            if args.do_half_precision:
