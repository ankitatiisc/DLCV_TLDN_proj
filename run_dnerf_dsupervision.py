...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
                  ...
                  use_viewdirs=False, c2w_staticcam=None, depths=None, #Added depth here
                  **kwargs):
    """Render rays
    Args:
...
def render_path(render_poses, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        rgb, disp, acc, depth, extras = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], frame_time=frame_time, **render_kwargs)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if N_rand is not None:
                ...
                batch_depth = torch.stack([rays_o, rays_d], 0) #Check if this is right to do
        ...
        batch_rays = torch.cat([batch_rays, batch_rays_depth], 1)

        #####  Core optimization loop  #####
        rgb, disp, acc, depth, extras = render(H, W, focal, chunk=args.chunk, rays=batch_rays, frame_time=frame_time,
        ...
        if i%args.i_img==0:
            ...
            frame_time = times[img_i]
            with torch.no_grad():
                rgb, disp, acc, depth, extras = render(H, W, focal, chunk=args.chunk, c2w=pose, frame_time=frame_time,
                                                    **render_kwargs_test)
            ...
            writer.add_image('depth', acc.cpu().numpy(), i, dataformats='HW')
            if 'rgb0' in extras:
