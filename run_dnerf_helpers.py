...
# Model
class DirectTemporalNeRF_WithLatentCodes(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, zero_canonical=True):
        super(DirectTemporalNeRF_WithLatentCodes, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
        ...
        self.embed_fn = embed_fn
        self.zero_canonical = zero_canonical
        #self.latent_code = nn.Embedding(1, 128) #embed dim 128
        self.input_time_latent = 128
        self._occ = NeRFOriginal(D=D, W=W, input_ch=input_ch, input_ch_views=input_ch_views,
                                 input_ch_time=input_ch_time, output_ch=output_ch, skips=skips,
                                 use_viewdirs=use_viewdirs, memory=memory, embed_fn=embed_fn, output_color_ch=3)
        self._time, self._time_out = self.create_time_net()
        self._net_time_latent = self.create_time_latent_net()

    def create_time_latent_net(self):
        return nn.Linear(self.input_time_latent, self.W)
    ...
    def query_time(self, new_pts, t, time_latent, net, net_final):
        idx = torch.tensor([0])
        h = torch.cat([new_pts, t], dim=-1)
        enc_time_latent = self._net_time_latent(time_latent(idx))
        #pdb.set_trace()
        for i, l in enumerate(net):
            h = net[i](h)
            h = F.relu(h)
    ...
    def forward(self, x, ts, time_latent):
        #pdb.set_trace()
        input_pts, input_views = torch.split(x, [self.input_ch, self.input_ch_views], dim=-1)
        t = ts[0]
        ...
        if cur_time == 0. and self.zero_canonical:
            ...
            dx = self.query_time(input_pts, t, time_latent, self._time, self._time_out)
            input_pts_orig = input_pts[:, :3]
            input_pts = self.embed_fn(input_pts_orig + dx)
        out, _ = self._occ(torch.cat([input_pts, input_views], dim=-1), t)
        return out, dx
...
class NeRF:
    ...
    def get_by_name(type,  *args, **kwargs):
        ...
        elif type == "direct_temporal_latent":
            model = DirectTemporalNeRF_WithLatentCodes(*args, **kwargs)
        else:
