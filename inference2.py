...
def batchify(fn, chunk):
    ...
    if chunk is None:
        return fn
    def ret(inputs_pos, inputs_time, time_latent):
        num_batches = inputs_pos.shape[0]

        out_list = []
        dx_list = []
        for i in range(0, num_batches, chunk):
            out, dx = fn(inputs_pos[i:i+chunk], [inputs_time[0][i:i+chunk], inputs_time[1][i:i+chunk]], time_latent)
            out_list += [out]
            dx_list += [dx]
...
def run_network(inputs, viewdirs, frame_time, time_latent, fn, embed_fn, embeddirs_fn, embedtime_fn, netchunk=1024*64,
    ...
    outputs_flat, position_delta_flat = batchify(fn, netchunk)(embedded, embedded_times, time_latent)
...
def create_nerf(args, time_latent, freeze_params):
    ...
    network_query_fn = lambda inputs, viewdirs, ts, network_fn : run_network(inputs, viewdirs, ts, time_latent, network_fn,
                                                                embed_fn=embed_fn,
    ...
    if args.ft_path is not None and args.ft_path!='None':
        ckpts = [args.ft_path]
    elif args.model_path is not None and args.model_path!='None':
...
def make_time_latent(init=None):
    embdim = 128
    d = 1
    time_codes = nn.Embedding(d, embdim)
    if None:
        time_codes.weight = nn.Parameter(torch.randn(d, embdim) / math.sqrt(embdim/2))
    else:
        time_codes.weight = nn.Parameter(torch.from_numpy(init))
    return time_codes
...
def train():
    ...
    #init_latent = (np.load('estimated_time_latent_lego_23_t2.npy')).astype(np.float32)
    #final_latent = (np.load('estimated_time_latent_lego_29_t1.npy')).astype(np.float32)

    init_latent = (np.load('estimated_time_latent_bballs_r001.npy')).astype(np.float32)
    final_latent = (np.load('estimated_time_latent_bballs_r148.npy')).astype(np.float32)

    time_codes = make_time_latent(init=init_latent)
    time_codes.to(device)

    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_nerf(args, time_codes,freeze_params=True)
    global_step = start
    
    optimizer_latent_code = torch.optim.Adam(params=time_codes.parameters(), lr=args.lrate, betas=(0.9, 0.999))
    ...
    result_save_dir = './project_results'
    exp_name = 'latent_init_bballs_001'
    save_path_name = os.path.join(result_save_dir, exp_name)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            cv2.imwrite(os.path.join(save_path_name, 'image_init.png'), image.astype(np.uint8)[:,:,::-1])
            #pdb.set_trace()
            #imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=30, quality=8)


            #End position
            #time_codes = make_time_latent(init=final_latent)
            time_codes.weight = nn.Parameter(torch.from_numpy(final_latent))
            time_codes.to(device)
            ...
            image = np.clip(image, 0, 255)
            cv2.imwrite(os.path.join(save_path_name, 'image_final.png'), image.astype(np.uint8)[:,:,::-1])
            #pdb.set_trace()
            print('Move Latent in a direction')
            direction = init_latent.copy()
            norm = np.linalg.norm(direction)
            unit_direction = direction / norm
            for alpha in np.arange(0.1,1.1,0.1):
                new_direction = init_latent + alpha * unit_direction
                #new_direction = np.expand_dims(new_direction, axis=0)
                #print(new_direction.shape)
                #new_direction = torch.FloatTensor(new_direction)
                #time_codes.from_pretrained(new_direction)
                #time_codes = make_time_latent(init=new_direction)
                time_codes.weight = nn.Parameter(torch.from_numpy(new_direction))
                time_codes.to(device)
                ...
                image = image * 255
                image = np.clip(image, 0, 255)
                cv2.imwrite(os.path.join(save_path_name, 'image_{:.1f}.png'.format(alpha)), image.astype(np.uint8)[:,:,::-1])
                #pdb.set_trace()

                imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=30, quality=8)

            print('Move Latent between inti and final direction')
            direction = final_latent - init_latent
            for alpha in np.arange(0.1,1.1,0.1):
                new_direction = init_latent + alpha * direction
                #new_direction = np.expand_dims(new_direction, axis=0)
                #print(new_direction.shape)
                #new_direction = torch.FloatTensor(new_direction)
                #time_codes.from_pretrained(new_direction)
                ...
                image = image * 255
                image = np.clip(image, 0, 255)
                cv2.imwrite(os.path.join(save_path_name, 'image_diff_{:.1f}.png'.format(alpha)), image.astype(np.uint8)[:,:,::-1])
                #pdb.set_trace()
    ...
    #mean time latent
    mean_time_latent = time_codes(torch.tensor([0])).detach().cpu().numpy()
    wt_mean_time_latent = 1
    start = start + 1
    for i in trange(start, N_iters):
        time0 = time.time()

        # Sample random ray batch
        #print('Sample random ray batches')
        if use_batching:
        ...
        #print('Sample random ray batches finished! Batch Size ', batch_rays.shape )
        #####  Core optimization loop  #####
        rgb, disp, acc, extras = render(H, W, focal, chunk=args.chunk, rays=batch_rays, frame_time=frame_time,
                                                verbose=i < 10, retraw=True,
                                                **render_kwargs_train)

        #print('Core step for optimization')
        if args.add_tv_loss:
        ...
        optimizer_latent_code.zero_grad()
        img_loss = img2mse(rgb, target_s)
        reg_loss = torch.mean(torch.norm(time_codes(torch.tensor([0])), dim=-1))
        tv_loss = 0
        ...
        loss = img_loss + tv_loss + reg_loss
        if i % args.i_print == 0:
            print('Iteration : {}, Image Loss : {}, Reg Loss : {}, Total loss : {}'.format(
                i, img_loss.item(), reg_loss.item(), loss.item() ))
        psnr = mse2psnr(img_loss)
        #print('Loss : {} \t PSNR : {}'.format(loss.item(), psnr.item()))
        if 'rgb0' in extras:
        ...
        for param_group in optimizer_latent_code.param_groups:
            param_group['lr'] = new_lrate
        ################################
        ...
        #Save latent code every iteration
        #torch.save( time_codes.state_dict() ,'time_latent.pt')
        #pdb.set_trace()
        ...
        global_step += 1
    estimated_time_latent  = time_codes(torch.tensor([0])).detach().cpu().numpy()
    np.save('estimated_time_latent.npy',estimated_time_latent)
