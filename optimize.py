...
def train():
    ...
    zero_arr = np.zeros((1,128), dtype=np.float32)
    init_code = (np.load('mean_latent_bballs_1L.npy')).astype(np.float32)
    time_codes = make_time_latent(init=init_code)
    #time_codes = make_time_latent(init=zero_arr)
    ...
    for i in trange(start, N_iters):
        ...
        reg_loss = 1e-4 * torch.mean(torch.norm(time_codes(torch.tensor([0])), dim=-1))
        tv_loss = 0
